...
if __name__ == '__main__':
    ...
    class MyProcessor(processor.ProcessorABC):
        ...
        def process(self, events):
            ...
            # hcr = (
            #     dhist.Hist.new
            #     .Reg(100, 0.5, 4.5, name='color_ring', label='Color_Ring')
            #     .Weight()
            # )
            # fill_cr = ak.fill_none(ak.flatten(boosted_fatjet.color_ring), 0)
            # hcr.fill(color_ring=fill_cr)

            # uf_cr_var = ak.unflatten(color_ring(boosted_fatjet, groomed=True), counts=ak.num(boosted_fatjet))
            ...
            # mosm = (
            #     dhist.Hist.new
            #     .Reg(40, 0.9, 1.5, name='MRatio', label='MRatio')
            #     .Weight()
            # )
    ...
    print("300")
    result["QCD_Pt_300to470_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q347"])
    print("470")
    result["QCD_Pt_470to600_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q476"])
    print("600")
    result["QCD_Pt_600to800_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q68"])
    print("800")
    result["QCD_Pt_800to1000_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q810"])
    print("1000")
    result["QCD_Pt_1000to1400_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q1014"])
    print("1400")
    result["QCD_Pt_1400to1800_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q1418"])
    print("1800")
    result["QCD_Pt_1800to2400_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q1824"])
    print("2400")
    result["QCD_Pt_2400to3200_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q2432"])
    print("3200")
    result["QCD_Pt_3200toInf_TuneCP5_13TeV_pythia8"] = MyProcessor().process(events["q32Inf"])
    stop = time.time()
    print(stop-start)
